...
| settings | MapSettings | yes | Seed values (name/icon/scope/type + toggles + tagScheme/homeMapSystemId) from `loadMapSettings`. |
...
### Renders
A tabbed dialog (`Tabs`): **General** (name + icon inputs, read-only scope/visibility), **Settings** (toggle checkboxes), **Tagging** (owner/admin only — scheme select + Home picker), **Export** (download button), **Import** (file picker).
...
- Tagging Save → `updateMapSettingsAction({ mapId, tagScheme, homeMapSystemId })` (owner/admin-gated server-side). Home picker disabled when scheme is `Off`; empty Home selection sends `null`. Config propagates to other clients on next map load (not realtime).
