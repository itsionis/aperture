...
### Renders
A tabbed dialog (`Tabs`): **General** (name + icon inputs, read-only scope/visibility), **Settings** (per-device display preferences — currently a low-contrast theme toggle), **Export** (download button), **Import** (file picker).
