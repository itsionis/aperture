...
**Purpose:** Four-tab live snapshot of the open map — Summary, Systems, Connections, Pilots.
...
- The online-pilot count and Pilots roster come from the map-wide presence store, so they track realtime `characterUpdate` movement while the dialog is mounted.
- The Pilots-tab location resolves each pilot's `systemId` against `viewData.systems` (via `systemNameById`) for the class label / name / tag; a pilot in a system not on the map falls back to the raw id.
...
- `systemClassColor` from `@/components/map/styling` (class-label colour on the Pilots tab)
