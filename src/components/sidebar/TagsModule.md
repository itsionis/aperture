...
### Renders
A compact card. ABC → exactly six rows (C1–C6), each coloured with `systemClassColor` matching the map node palette, showing the next 3 available letters. 0121 → next root child off Home plus the selected system's next child. Renders nothing when `viewData.map.tagScheme === 'none'`.
...
- `systemClassColor` (`@/components/map/styling`) — per-row colour matching the map node palette.
- `Card` primitives.
