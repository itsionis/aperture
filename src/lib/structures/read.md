...
### type StructureIntel
A structure row shaped for the sidebar: `id`/ids as strings, `typeName` resolved from `universe_type`, `createdByName` from `ap_character`, timestamps as ISO strings.
...
Structure intel for the given universe systems, keyed by `system_id`. One batched query joins `universe_type` (type name) and `ap_character` (creator name). Empty input → `{}`; systems with no structures are absent.
...
Shapes a freshly written `ap_structure` row into a complete `StructureIntel` (resolving `typeName`/`createdByName`) so create/update routes return a spliceable row to the client.
