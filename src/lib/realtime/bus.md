...
- On notification, parses the payload as JSON (non-JSON → `{}`), derives `mapId` from the channel name, and emits one of two envelopes:
  ...
  - **`{ task: 'systemNotification', load }`** — when the pg_notify payload has a top-level `task: 'systemNotification'` (Stage 17.8 zKB feed broadcast). The bus validates the `load` against `systemNotificationLoadSchema` and drops malformed envelopes silently.
  - **`{ task: 'connectionMassLog', load }`** — when the pg_notify payload has a top-level `task: 'connectionMassLog'` (Stage 17.11a mass-log broadcast). The bus validates the `load` against `connectionMassLogLoadSchema` and drops malformed envelopes silently.
