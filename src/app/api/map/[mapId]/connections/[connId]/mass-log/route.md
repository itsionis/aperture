...
Returns `{ ok: true, data: ConnectionMassLogEntry[] }` — the connection's logged jumps oldest-first
with a running cumulative mass (see `listConnectionMassLog` in `src/lib/map/connectionMassLog.ts`).
