...
### Depends On
- `@/lib/map/loadMap`, `@/lib/map/routeConfig`, `@/lib/map/stats`, `@/lib/map/intel`, `@/lib/structures/read`, `@/lib/session` (`getAccountCharacters`, `getConnectionTravelAnimation`, `getMainCharacterId`, `getMapLayout`, `getSignatureIndicatorPrefs`, `requireSession`), `@/components/map/MapCanvas`, `@/components/ui/card`. Behavior toggles and auto-tagging are admin-only; this page no longer checks `isMapOwnerOrAdmin`.
