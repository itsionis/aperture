...
- **Task vocabulary** inherited verbatim from [04 § Realtime transport coverage](10-feature-matrix.md#realtime-transport-coverage-stage-d) — `mapUpdate`, `mapAccess`, `mapConnectionAccess`, `mapDeleted`, `characterUpdate`, `characterLogout`, `healthCheck`, `logData`, `subscribe`, `unsubscribe`. The rebuild adds `systemNotification` (Stage 17.8) — a transient server-observed system event (zKillboard kills) broadcast like `characterUpdate` via direct `pg_notify`, bypassing `ap_map_event`. Payload shapes are a Phase-0 deliverable (§11).
