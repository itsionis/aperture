...
- Then, when `env.ZKB_FEED_ENABLED` (default true), starts the long-lived zKillboard feed via `startZkbFeed()` (Stage 17.8). It's a single global loop (not a per-entity job), so it lives here beside the worker rather than in graphile-worker.
